class Event {
  ...
  async getEventsCategories(page = 1, per_page = 6) {
    try {
        const token = 'Basic ' + Buffer.from(this.SEATGEEK_CLIENT_ID + ':' + this.SEATGEEK_SECRET_TOKEN).toString('base64');
        const response = await fetch(`https://api.seatgeek.com/2/taxonomies?page=${page}&per_page=${per_page}`, {
          method: 'GET',
          headers: {
            Authorization:  token
          }
  ...
  async getSearchEvents(query, page = 1, per_page = 10) {
    try {
        const token = 'Basic ' + Buffer.from(this.SEATGEEK_CLIENT_ID + ':' + this.SEATGEEK_SECRET_TOKEN).toString('base64');
        const response = await fetch(`https://api.seatgeek.com/2/events?page=${page}&per_page=${per_page}&q=${query}`, {
          method: 'GET',
          headers: {
            Authorization:  token
          }
